...
import { generateNutritionContent, getRelatedImages, GeneratedContent } from '@/services/ai-service';
import { useHistoryActions } from '../contexts/history-context';
// 移除 useTeacher 的导入
import TeacherSetup from './teacher-setup';

// 本地存储键名
const GENERATED_CONTENT_KEY = 'nutrition_generated_content';
const LAST_COMMENT_KEY = 'nutrition_last_comment';

const ConsultantDashboard: React.FC = () => {
  const { isMobile, isTablet, isDesktop } = useMobile();
  const { addNutritionConsultRecord, addContentViewRecord } = useHistoryActions();
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 调用AI服务生成内容
      const content = await generateNutritionContent(comment);
      ...
      // 保存到历史记录
      addNutritionConsultRecord(comment, {
  ...
  const handleSaveText = (id: number) => {
    ...
    if (content) {
      try {
        ...
        link.href = url;
        link.download = `营养咨询-${content.title}.txt`;
        link.click();
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F5F5F7] dark:bg-[#1C1C1E]">
      <Header />
        ...
        >
          ...
          <Card className="mb-8 shadow-sm border-0 dark:bg-gray-800 dark:border-gray-700">
            <CardContent className="p-6">
              <h2 className="text-xl font-semibold mb-4 text-[#1D1D1F] dark:text-white">输入营养咨询问题</h2>
              <Textarea
                placeholder="请输入观众的问题或评论..."
                className="min-h-[120px] mb-4 border-[#D2D2D7] focus:border-[#007AFF] focus:ring-[#007AFF] dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                value={comment}
                onChange={(e) => {
              ...
              <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:justify-between">
                ...
                <Button 
                  ...
                  className="w-full sm:w-auto bg-[#007AFF] hover:bg-[#0066CC] text-white"
                >
                  {isGenerating ? (
                    <>
          ...
          {generatedContent.length === 0 && !isGenerating && (
            <div className="text-center py-16">
              ...
              <p className="text-[#86868B] dark:text-gray-400 max-w-md mx-auto">
                输入观众的饮食或营养问题，点击"生成解析"按钮，系统将为您生成相关的营养学解析和饮食建议。
              </p>
            </div>
          )}
          ...
          {isGenerating && generatedContent.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-[#86868B] dark:text-gray-400 max-w-md mx-auto">
                系统正在分析您输入的问题，并生成相关的营养学解析和饮食建议，请稍候...
              </p>
            </div>
          )}
